...
def find_vines_in_vinerow(entity_type, vinerow_id):
    # Orion Context Broker API base URL
    FIWARE_ORION_BASE_URL = "http://localhost:1026/v2/entities/"

    try:
        # Query Orion to get all entities of type Vine with the specified vinerow_id
        fiware_orion_url = f"{FIWARE_ORION_BASE_URL}?type={entity_type}&q=vinerow_id=={vinerow_id}"
...
vine_data_vinerow = find_vines_in_vinerow("Vine", vinerow_id)

if vine_data_vinerow is not None:
    # Find all photo URLs associated with the vines in vinerow
    print("Photo URLs associated with vines in " + vinerow_id + ":")
